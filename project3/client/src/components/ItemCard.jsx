...
export function ItemCard({ item, showActions = false }) {
  ...
  const handleClaim = () => {
    if (state.user) {
      updateItemStatus(item.id, 'Claimed', state.user.id, state.user.name);
    }
  };

  const canClaim =
    item.status === 'Approved' &&
    item.createdBy !== state.user?.id &&
    ...
    <div className="bg-white rounded-lg shadow-md hover:shadow-lg transition-shadow duration-300 overflow-hidden">
      ...
      <div className="p-6">
        ...
        <div className="space-y-2 mb-4">
          ...
          {item.status === 'Claimed' && item.claimantName && (
            <div className="flex items-center text-sm text-gray-500">
              <Tag className="w-4 h-4 mr-2" />
              <span>Claimed by {item.claimantName}</span>
            </div>
          )}
        </div>
