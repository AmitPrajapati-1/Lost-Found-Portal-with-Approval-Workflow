    ...
    const fetchApproved = async () => {
      try {
        ...
        const normalized = data.map((i) => ({
          ...
          location: i.location,
          status: i.status,
          createdBy: i.reportedBy?._id,
          createdByName: i.reportedBy?.name,
  ...
  const addItem = async (itemData) => {
    try {
      ...
      const normalized = {
        ...
        date: saved.dateReported,
        location: saved.location,
        status: saved.status === 'Pending' ? 'PendingApproval' : saved.status,
        createdBy: saved.reportedBy,
        createdByName: undefined,
        createdAt: saved.dateReported,
    ...
    try {
      if (status === 'Approved' || status === 'Rejected' || status === 'Pending') {
        const res = await fetch(`${API_BASE}/items/${itemId}`, {
          ...
          body: JSON.stringify({ status: status === 'PendingApproval' ? 'Pending' : status }),
        });
        if (!res.ok) throw new Error('Failed');
        ...
        dispatch({
          ...
          payload: {
            ...item,
            status: updated.status === 'Pending' ? 'PendingApproval' : updated.status,
          },
        });
      ...
      if (status === 'PendingApproval') toast.error(`Item "${item.title}" has been rejected.`);
    } catch (e) {
